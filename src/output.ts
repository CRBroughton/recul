...
export function printResults({ results, lag, pm, behindBehavior, rangeSpecifier, workspaceFile, fixed }: PrintResultsOptions): void {
  ...
  for (const r of sorted) {
    ...
    if (r.status === 'pin')                                        statusLabel = '↓ will pin back';
    else if (r.status === 'ok')                                    statusLabel = '✓ ok';
    else if (r.status === 'behind' && behindBehavior === 'report') statusLabel = '↑ safe to upgrade';
    else if (r.status === 'behind')                                statusLabel = '✓ ok';
  ...
  if (mismatches.length > 0) {
    ...
    if (mismatchesOnly.length > 0) console.log(`   Re-pin command included below.`);
  }
