...
export { parseNpmLock, npmAdapter } from './adapters/npm.js';
export { parsePnpmLock, pnpmAdapter, loadPnpmCatalog, resolveCatalogRefs, updatePnpmCatalog } from './adapters/pnpm.js';
...
export function detectPackageManager(dir: string): PackageManager | null {
  for (const [filename, pm] of PM_LOCKFILES) {
    if (existsSync(resolve(dir, filename))) return pm;
  }
